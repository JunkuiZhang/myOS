...
PageTableManager *OS_PAGETABLE_MANAGER;
...
void PageTableManager::mapMemory(void *virtual_memory, void *physical_memory) {
	...
	if (!pd_entry.getFlag(PageTableFlags::present)) {
		page_directory_pointer =
			(PageTable *)OS_PAGEFRAME_ALLOCATOR->requestPage();
	...
	if (!pd_entry.getFlag(PageTableFlags::present)) {
		page_directory = (PageTable *)OS_PAGEFRAME_ALLOCATOR->requestPage();
	...
	if (!pd_entry.getFlag(PageTableFlags::present)) {
		page_table = (PageTable *)OS_PAGEFRAME_ALLOCATOR->requestPage();
