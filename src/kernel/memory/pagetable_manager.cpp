...
void PageTableManager::mapMemory(void *virtual_memory, void *physical_memory,
	...
	if (!pd_entry.present) {
		...
		pd_entry.address = (uint64_t)page_directory_pointer >> 12;
		...
		page_directory_pointer =
			(PageTable *)((uint64_t)pd_entry.address << 12);
	}
	...
	if (!pd_entry.present) {
		...
		pd_entry.address = (uint64_t)page_directory >> 12;
		pd_entry.present = true;
		pd_entry.read_write = true;
		page_directory_pointer->entries[pagemap_indexer.page_directory_index] =
			pd_entry;
	} else {
		page_directory = (PageTable *)((uint64_t)pd_entry.address << 12);
	}
	...
	if (!pd_entry.present) {
		...
		pd_entry.address = (uint64_t)page_table >> 12;
		pd_entry.present = true;
		pd_entry.read_write = true;
		page_directory->entries[pagemap_indexer.page_table_index] = pd_entry;
	} else {
		page_table = (PageTable *)((uint64_t)pd_entry.address << 12);
	}

	pd_entry = page_table->entries[pagemap_indexer.page_index];
	pd_entry.address = (uint64_t)physical_memory >> 12;
	pd_entry.present = true;
	pd_entry.read_write = true;
