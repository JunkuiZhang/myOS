...
const char *massStorageControllerSubclassName(uint8_t subclass_code) {
...
const char *bridgeDeviceSubclassName(uint8_t subclass_code) {
	switch (subclass_code) {
	case 0x00:
		return "Host Bridge";
	case 0x01:
...
const char *serialBusControllerSubclassName(uint8_t subclass_code) {
	switch (subclass_code) {
	case 0x00:
...
const char *getSubClassName(uint8_t class_code, uint8_t subclass_code) {
	switch (class_code) {
	case 0x01:
		return massStorageControllerSubclassName(subclass_code);
	case 0x03:
		switch (subclass_code) {
		case 0x00:
	...
	case 0x06:
		return bridgeDeviceSubclassName(subclass_code);
	case 0x0C:
		return serialBusControllerSubclassName(subclass_code);
	}
	return "Unknown";
}
